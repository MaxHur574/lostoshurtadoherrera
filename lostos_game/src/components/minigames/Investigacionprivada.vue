<template>
  <Window title="📁 Investigación Privada" @close="$emit('close')" @minimize="$emit('minimize')">
    <div class="inv-app">
      ...
      <div class="file-list">
        <div
          ...
          :key="nota.key"
          class="file-item"
          :class="{ locked: !mg.unlocked[nota.key], selected: selected === nota.key }"
          @click="selected = nota.key"
          @dblclick="openNota(nota)"
        >
          <span class="file-icon">{{ mg.unlocked[nota.key] ? '📄' : '🔒' }}</span>
          <div class="file-info">
            <div class="file-name">{{ nota.filename }}</div>
            <div class="file-sub">
              {{ mg.unlocked[nota.key] ? 'Desbloqueado · doble clic para leer' : 'Bloqueado · doble clic para desbloquear' }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="file-preview">
        ...
        <div v-else class="preview-info">
          ...
          <div class="pi-row"><span>Estado</span><span>{{ mg.unlocked[currentNota?.key] ? '🟢 Desbloqueado' : '🔴 Bloqueado' }}</span></div>
          <div class="pi-row"><span>Tipo</span><span>.val — archivo cifrado</span></div>
          <div class="pi-row"><span>Autor</span><span>E.V.</span></div>
          <div class="pi-hint">
            {{ mg.unlocked[currentNota?.key] ? 'Doble clic para leer el contenido.' : 'Doble clic para iniciar el minijuego de desbloqueo.' }}
          </div>
        </div>
      </div>
...
const currentNota = computed(() =>
  notas.find(n => n.key === selected.value)
);
...
.file-item.locked   { opacity: .75; }

.file-icon { font-size: 22px; flex-shrink: 0; }
.file-name { font-size: 12px; color: #ccc; }
.file-sub  { font-size: 10px; color: #444; margin-top: 2px; }
.file-item.locked .file-sub { color: #6a2a2a; }
...
.pi-row span:last-child { color: #777; font-family: 'Courier New', monospace; }
