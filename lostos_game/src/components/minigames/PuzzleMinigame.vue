...
onMounted(() => {
  let arr;
  do { arr = shuffle([0,1,2,3,4,5,6,7,8]); }
...
function pick(i) {
  ...
  if (sel.value === null) {
    ...
    [pieces.value[i], pieces.value[sel.value]];
    sel.value = null;
...
.piece:hover    { border-color: #3a3a3a; }
.piece.selected { border-color: #4a9eff; }
.piece.correct  { border-color: #2a5a2a; }
.win-msg {
