<template>
  <div class="overlay" @click.self="$emit('close')">
    <div class="mg-box">
      ...
      <div class="mg-body">
        <p class="hint">Haz clic en dos fragmentos para intercambiarlos. Reconstruye la noticia en orden.</p>
...
function pick(i) {
  ...
  if (sel.value === null) {
    ...
    [frags.value[i], frags.value[sel.value]];
    sel.value = null;
...
.hint { font-size: 11px; color: #555; font-family: 'Segoe UI', sans-serif; margin: 0 0 12px; }
.frags-list { display: flex; flex-direction: column; gap: 6px; }
.frag {
...
.frag:hover    { background: #1e1e1e; border-color: #3a3a3a; }
.frag.selected { border-color: #4a9eff; background: #0f1e2a; }
.frag.correct  { border-color: #2a5a2a; }
.frag-num  { font-size: 10px; color: #444; font-family: 'Courier New', monospace; flex-shrink: 0; width: 14px; }
.frag-text { font-size: 12px; color: #999; font-family: 'Courier New', monospace; }
.frag.correct .frag-text { color: #3a7a3a; }
.win-msg {
