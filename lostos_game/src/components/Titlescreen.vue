  ...
  <div class="os-wrapper">
        ...
        <div class="mail-client">
          ...
          <div class="mail-sidebar">
            ...
            <div class="sidebar-folders">
              ...
              <div class="folder-item"><span>⭐</span><span>Destacados</span></div>
              <div class="folder-item"><span>📤</span><span>Enviados</span></div>
              <div class="folder-item"><span>📝</span><span>Borradores</span></div>
              <div class="folder-item"><span>🗑️</span><span>Papelera</span></div>
            </div>
          </div>
            ...
            <div v-else class="mystery-mail-content">
              ...
              <div class="mystery-body">
                ...
                <div v-if="!activePanel" class="game-menu">
                  <div class="gm-cursor-line"><span class="cursor-blink">_</span></div>
                  ...
                  <div class="opt-row" v-for="opt in options" :key="opt.id">
                    ...
                    <div class="opt-ctrl">
                      <template v-if="opt.type === 'slider'">
                        <input type="range" min="0" max="100" v-model="opt.value" class="opt-slider" />
                        <span class="opt-val">{{ opt.value }}%</span>
                      </template>
                      <template v-else-if="opt.type === 'toggle'">
                        <button class="opt-toggle" :class="{ on: opt.value }" @click="opt.value = !opt.value">
                          <span class="ott" /><span class="oth" />
                        </button>
                        <span class="opt-val">{{ opt.value ? 'ON' : 'OFF' }}</span>
                      </template>
                      <template v-else-if="opt.type === 'select'">
                        <select class="opt-select" v-model="opt.value">
                          <option v-for="c in opt.choices" :key="c.val" :value="c.val">{{ c.label }}</option>
                        </select>
                      </template>
                    </div>
                  </div>
                ...
                <div v-else-if="activePanel === 'credits'" class="sub-panel">
                  ...
                  <div class="credits-list">
                    ...
                    <div class="credit-quote">"No confíes en todo lo que encuentres.<br/>Algunas cosas parecen pistas y no lo son."</div>
    ...
    <div class="infect-screen" v-if="infecting">
      <div class="infect-lines">
        <div v-for="(line, i) in visibleInfectLines" :key="i" class="infect-line" :class="line.type">{{ line.text }}</div>
      </div>
      <div class="infect-progress-bar">
        <div class="infect-fill" :style="{ width: infectProgress + '%' }" />
      </div>
      <div class="infect-status">SISTEMA COMPROMETIDO — {{ Math.round(infectProgress) }}%</div>
    </div>
...
const menuItems = [
  ...
  { id: "options", icon: "⚙", label: "OPCIONES",     desc: "Audio, video, controles",      key: null },
...
const options = ref([
  { id: "sfx",       label: "Volumen efectos",  desc: "Sonidos del sistema",        type: "slider", value: 70 },
  { id: "amb",       label: "Volumen ambiente",  desc: "Música y ambientación",      type: "slider", value: 45 },
  ...
  { id: "lang",      label: "Idioma",            desc: "Idioma del sistema",         type: "select", value: "es",
    choices: [{ val: "es", label: "Español" }, { val: "en", label: "English" }, { val: "??", label: "???" }] },
]);
...
function playDesktopMusic() {
  // placeholder — conectar sistema de audio aquí cuando esté disponible
}
...
function startInfection() {
  ...
  setTimeout(() => { emit("started"); }, allInfectLines.length * 220 + 800);
}
...
.desk-icon-img   { font-size: 30px; }
.desk-icon-label { font-size: 11px; color: #fff; text-shadow: 0 1px 4px rgba(0,0,0,0.9); text-align: center; }

.mail-window {
  position: absolute;
  top: 24px; left: 110px; right: 16px; bottom: 52px;
  background: #fafafa;
...
.mail-window.shake { animation: window-shake 0.5s ease; }
...
.win-titlebar-left { display: flex; align-items: center; gap: 7px; }
.win-app-icon  { font-size: 16px; }
.win-app-title { font-size: 12px; color: #333; }
...
.mail-client { flex: 1; display: flex; overflow: hidden; background: #fff; }
...
.sidebar-account {
  ...
  padding: 14px 12px; border-bottom: 1px solid #dde3ea;
}
...
.acc-name  { font-size: 13px; font-weight: 600; color: #1a1a2e; }
.acc-email { font-size: 10px; color: #888; }

.sidebar-folders { padding: 6px 4px; }

.folder-item {
  display: flex; align-items: center; gap: 8px;
  padding: 7px 10px; font-size: 13px; color: #444;
  cursor: pointer; border-radius: 6px; margin: 1px 0; transition: background 0.15s;
}
.folder-item:hover { background: rgba(0,0,0,0.05); }
.folder-item.active { background: #dce8f5; color: #1a4a8a; font-weight: 600; }

.badge { margin-left: auto; background: #c42b1c; color: #fff; border-radius: 10px; padding: 1px 6px; font-size: 10px; font-weight: bold; }
...
.mail-row:hover   { background: #f5f8ff; }
.mail-row.selected { background: #eaf1fb; }
.mail-row.read    { opacity: 0.7; }
...
@keyframes avatar-pulse {
  ...
  50%      { box-shadow: 0 0 0 4px rgba(204,51,51,0.2); }
}

.mail-row-body    { flex: 1; min-width: 0; }
.mail-row-from    { font-size: 12px; font-weight: 600; color: #222; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.mail-row-subject { font-size: 12px; color: #555; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 2px; }
.mail-row-preview { font-size: 11px; color: #aaa; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 2px; }
.mail-row-time    { font-size: 10px; color: #aaa; flex-shrink: 0; }

.mystery-from    { color: #cc3333 !important; font-family: 'Courier New', monospace; font-size: 11px !important; }
.mystery-subject { color: #888 !important; font-style: italic; }
.mystery-time    { color: #cc3333 !important; font-weight: 600; }

.unread-dot {
  position: absolute; left: 4px; top: 50%; transform: translateY(-50%);
  width: 6px; height: 6px; border-radius: 50%; background: #cc3333;
}
...
.mail-placeholder {
  ...
  align-items: center; justify-content: center; color: #ccc; gap: 10px;
}
.placeholder-icon { font-size: 40px; opacity: 0.3; }
.placeholder-text { font-size: 14px; }

/* ─── CORREO MISTERIOSO ─── */
.mystery-mail-content { height: 100%; display: flex; flex-direction: column; }
...
.mystery-subject-line { font-size: 18px; font-weight: 600; color: #888; font-style: italic; margin-bottom: 8px; }
.mystery-meta { display: flex; justify-content: space-between; font-size: 12px; color: #999; }
.mystery-sender strong { color: #cc3333; font-family: 'Courier New', monospace; font-size: 11px; }
.mystery-date { font-size: 11px; color: #bbb; }
.mystery-to   { font-size: 11px; color: #bbb; margin-top: 4px; }

.mystery-body { flex: 1; overflow-y: auto; padding: 16px 24px; background: #fff; }

/* ─── MENÚ DEL JUEGO ─── */
.game-menu { display: flex; flex-direction: column; gap: 2px; font-family: 'Courier New', monospace; }

.gm-cursor-line { font-size: 18px; color: #ddd; margin-bottom: 8px; }
...
.gm-item:hover, .gm-item.selected { background: #f5f0f0; border-color: rgba(204,51,51,0.12); }
.gm-item.disabled { opacity: 0.35; cursor: not-allowed; pointer-events: none; }

.gm-arrow { font-size: 10px; color: #cc3333; width: 12px; flex-shrink: 0; }
.gm-icon  { font-size: 15px; flex-shrink: 0; }
.gm-text  { flex: 1; display: flex; flex-direction: column; gap: 1px; }
.gm-name  { font-size: 13px; color: #222; font-family: 'Segoe UI', sans-serif; font-weight: 600; }
.gm-desc  { font-size: 11px; color: #aaa; font-family: 'Segoe UI', sans-serif; }
.gm-key   { font-size: 10px; color: #bbb; border: 1px solid #e0e0e0; padding: 2px 7px; border-radius: 4px; font-family: 'Courier New', monospace; flex-shrink: 0; }

/* ─── SUB-PANELES ─── */
.sub-panel { display: flex; flex-direction: column; gap: 0; font-family: 'Segoe UI', sans-serif; }
...
.sp-back:hover { color: #333; border-color: #aaa; }
.sp-title { font-size: 11px; color: #aaa; font-family: 'Courier New', monospace; letter-spacing: 1px; }
...
.opt-info  { flex: 1; }
.opt-label { font-size: 12px; color: #333; font-weight: 500; }
.opt-desc-small { font-size: 10px; color: #bbb; margin-top: 1px; }
.opt-ctrl  { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
.opt-val   { font-size: 11px; color: #888; min-width: 32px; text-align: right; }
...
.opt-slider::-webkit-slider-thumb {
  ...
  background: #cc3333; border-radius: 50%; cursor: pointer;
}
...
.ott {
  ...
  background: #e0e0e0; border-radius: 9px;
...
.opt-toggle.on .ott { background: #cc3333; }
.opt-toggle.on .oth { transform: translateX(18px); }

.opt-select {
  width: 110px; background: #f8f8f8; border: 1px solid #ddd;
  color: #333; padding: 4px 6px; border-radius: 4px;
  font-size: 11px; outline: none; cursor: pointer;
}

.sp-save {
  margin-top: 14px; background: #f5f5f5; border: 1px solid #ddd;
  color: #555; padding: 8px; border-radius: 4px; cursor: pointer;
  font-size: 11px; font-family: 'Courier New', monospace; letter-spacing: 1px; transition: background 0.15s;
}
.sp-save:hover { background: #eee; color: #222; }

.credits-list { display: flex; flex-direction: column; gap: 12px; font-family: 'Courier New', monospace; }
.credit-row   { display: flex; flex-direction: column; gap: 2px; }
.credit-role  { font-size: 9px; color: #bbb; letter-spacing: 1.5px; text-transform: uppercase; }
.credit-name  { font-size: 13px; color: #444; }
.credit-divider { border-top: 1px solid #f0f0f0; margin: 4px 0; }
.credit-quote { font-size: 12px; color: #aaa; font-style: italic; line-height: 1.7; font-family: 'Segoe UI', sans-serif; }
.credit-sig   { font-size: 11px; color: #bbb; }

/* ─── ADJUNTO ─── */
.attachment-zone {
  padding: 14px 24px 20px; border-top: 1px solid #eee;
  background: #f8f9fa; flex-shrink: 0;
}
.attachment-label { font-size: 10px; font-weight: 700; color: #aaa; letter-spacing: 1.5px; margin-bottom: 10px; }
...
.att-info { flex: 1; }
.att-name { font-size: 13px; font-weight: 600; color: #222; font-family: 'Courier New', monospace; }
.att-size { font-size: 11px; color: #888; margin-top: 2px; }

.download-btn {
  background: #0066cc; color: #fff; border: none; border-radius: 6px;
  padding: 7px 16px; font-size: 13px; cursor: pointer;
  font-family: 'Segoe UI', sans-serif; transition: background 0.2s, transform 0.1s;
  min-width: 120px; display: flex; align-items: center; justify-content: center; gap: 6px;
}
.download-btn:hover:not(:disabled) { background: #0052a3; transform: translateY(-1px); }
.download-btn:disabled { cursor: default; }
.download-btn.downloading { background: #333; }
.download-btn.done { background: #2a7a2a; }
...
.dl-fill { height: 100%; background: #4aaa4a; border-radius: 3px; transition: width 0.1s linear; }

.open-prompt {
  margin-top: 10px; font-size: 12px; color: #cc3333;
  font-family: 'Courier New', monospace;
  display: flex; align-items: center; gap: 6px; animation: fadeIn 0.3s ease;
}
.open-arrow { animation: pulse 0.5s infinite alternate; }
...
.win-taskbar {
  ...
  background: rgba(240,244,248,0.92); backdrop-filter: blur(20px);
...
.win-start:hover { background: rgba(0,0,0,0.06); }
.win-logo { font-size: 22px; }

.taskbar-middle { flex: 1; display: flex; align-items: center; justify-content: center; gap: 4px; }
.task-item {
  display: flex; align-items: center; gap: 6px;
  padding: 6px 12px; border-radius: 6px; font-size: 12px; color: #333;
  cursor: pointer; background: rgba(0,0,0,0.06); border-bottom: 2px solid #0066cc;
}

.taskbar-right { display: flex; align-items: center; gap: 10px; padding: 0 14px; }
.sys-icons { display: flex; gap: 6px; font-size: 14px; opacity: 0.6; }
.sys-clock { text-align: right; font-size: 11px; color: #333; line-height: 1.4; }
...
.infect-lines {
  ...
  gap: 3px; overflow: hidden; margin-bottom: 24px;
}

.infect-line { font-size: 13px; animation: lineIn 0.15s ease-out; }
@keyframes lineIn { from{opacity:0;transform:translateX(-8px);} to{opacity:1;transform:translateX(0);} }

.infect-line.ok    { color: #336633; }
.infect-line.warn  { color: #aa8800; }
.infect-line.err   { color: #cc3333; }
.infect-line.final { color: #ff4444; font-size: 15px; font-weight: bold; letter-spacing: 2px; text-shadow: 0 0 12px #ff4444; margin-top: 6px; }

.infect-progress-bar { height: 3px; background: #1a1a1a; border-radius: 2px; overflow: hidden; margin-bottom: 10px; }
.infect-fill { height: 100%; background: linear-gradient(90deg, #cc3333, #ff4444); transition: width 0.2s linear; box-shadow: 0 0 8px #ff4444; }
.infect-status { font-size: 11px; color: #444; letter-spacing: 2px; }
