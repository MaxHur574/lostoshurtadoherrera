<template>
  <Window title="🌐 Navegador" @close="$emit('close')" @minimize="$emit('minimize')">
    <div class="browser">
      ...
      <!-- Barra de navegación -->
      <div class="nav-bar">
        <button class="nav-btn" @click="goBack"    :disabled="historyStack.length <= 1">‹</button>
        <button class="nav-btn" @click="goForward" :disabled="forwardStack.length === 0">›</button>
      ...
      <div class="browser-content">
        ...
        <div v-if="currentPage?.id === 'newtab'" class="new-tab-page">
          ...
          <div class="new-tab-subtitle">Sin conexión · Solo caché local disponible</div>
        ...
        <div v-else-if="currentPage?.error" class="error-page">
          ...
          <div class="error-detail">Este equipo no tiene conexión a internet.<br/>Solo están disponibles las páginas en caché local.</div>
          <button class="error-retry" @click="reload">Reintentar</button>
        </div>

        <!-- Páginas internas -->
        <div v-else-if="currentPage" class="site-page">
          <div class="site-header" :style="{ background: currentPage.headerBg || '#0a0a0a' }">
...
// ── Historial narrativo ───────────────────────────────────────────────────────
const historial = [
  { icon: '🔍', label: 'recuperar acceso cuenta bloqueada',  url: 'buscar:recuperar+acceso+cuenta+bloqueada',  date: '29/10/2020' },
  { icon: '🔍', label: 'mercado rojo darknet',               url: 'buscar:mercado+rojo+darknet',               date: '14/05/2024' },
  { icon: '🔍', label: 'desapariciones zona industrial',     url: 'buscar:desapariciones+zona+industrial',     date: '10/03/2022' },
  { icon: '🌐', label: 'Recuperación de cuenta — lost.sys',  url: 'lost.sys/cache/recuperacion',               date: '29/10/2020' },
  { icon: '🔴', label: 'mercadorojo.onion — caché local',    url: 'lost.sys/cache/mercadorojo',                date: '14/05/2024' },
];

// ── Favoritos ─────────────────────────────────────────────────────────────────
const favoritos = [
  { icon: '🔑', label: 'Recuperación de cuenta', url: 'lost.sys/cache/recuperacion' },
...
function navigate(url) {
  if (!url || url === 'about:blank' || url === '') { setPage('newtab'); return; }
  loading.value = true;
  setTimeout(() => {
    loading.value = false;
    ...
    if (pages[key]) {
      ...
      const tab = tabs.value.find(t => t.id === currentTabId.value);
      if (tab) { tab.pageId = errKey; tab.title = 'Sin conexión'; tab.icon = '⚠️'; }
    }
  }, 500);
}
...
function goBack() {
  ...
  if (tab && prev) {
    ...
    if (page) { tab.title = page.title; tab.icon = page.icon; urlValue.value = page.url === 'about:blank' ? '' : page.url; }
  }
}
...
function closeTab(id) {
  ...
  if (currentTabId.value === id) currentTabId.value = tabs.value[Math.max(0, idx - 1)].id;
}
</script>

<style scoped>
.browser { display: flex; flex-direction: column; height: 500px; font-family: 'Segoe UI', sans-serif; background: #0d0d0d; }

/* Nav */
.nav-bar { display: flex; align-items: center; gap: 4px; padding: 6px 8px; background: #141414; border-bottom: 1px solid #222; }
.nav-btn { background: #1a1a1a; border: 1px solid #2a2a2a; color: #888; width: 26px; height: 26px; border-radius: 4px; cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center; transition: background .15s; flex-shrink: 0; }
.nav-btn:hover:not(:disabled) { background: #222; color: #ccc; }
.nav-btn:disabled { opacity: 0.3; cursor: default; }
.go-btn { font-size: 12px; }
.url-bar { flex: 1; display: flex; align-items: center; gap: 6px; background: #0d0d0d; border: 1px solid #333; border-radius: 4px; padding: 0 8px; height: 26px; }
.url-bar:focus-within { border-color: #555; }
.url-bar.error { border-color: #5a2a2a; }
.url-icon { font-size: 11px; opacity: 0.6; flex-shrink: 0; }
.url-input { flex: 1; background: none; border: none; color: #ccc; font-size: 12px; outline: none; font-family: 'Courier New', monospace; }

/* Tabs */
.tabs-bar { display: flex; align-items: center; background: #111; border-bottom: 1px solid #222; overflow-x: auto; min-height: 32px; }
.tab { display: flex; align-items: center; gap: 5px; padding: 0 10px; height: 32px; min-width: 100px; max-width: 180px; cursor: pointer; color: #666; font-size: 12px; border-right: 1px solid #1a1a1a; transition: background .15s; flex-shrink: 0; }
...
.tab-close  { background: none; border: none; color: #444; font-size: 10px; cursor: pointer; padding: 2px 3px; border-radius: 3px; transition: background .15s; flex-shrink: 0; }
.tab-close:hover { background: #2a2a2a; color: #ff6666; }
.new-tab-btn { background: none; border: none; color: #555; font-size: 16px; cursor: pointer; padding: 0 10px; height: 32px; transition: color .15s; flex-shrink: 0; }
...
/* Nueva pestaña */
.new-tab-page { padding: 24px 20px; display: flex; flex-direction: column; gap: 20px; }
.new-tab-logo     { font-size: 20px; color: #2a2a2a; font-weight: bold; font-family: 'Courier New', monospace; }
.new-tab-subtitle { font-size: 11px; color: #222; font-family: 'Courier New', monospace; margin-top: -14px; }

.nt-section { display: flex; flex-direction: column; gap: 6px; }
.nt-title   { font-size: 10px; color: #333; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px; }

.history-item { display: flex; align-items: center; gap: 10px; padding: 7px 10px; background: #0f0f0f; border: 1px solid #1a1a1a; border-radius: 4px; cursor: pointer; transition: background .15s; }
...
.h-label { font-size: 12px; color: #555; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.quick-link  { display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 10px 14px; background: #0f0f0f; border: 1px solid #1a1a1a; border-radius: 6px; cursor: pointer; transition: background .15s; min-width: 80px; }
...
/* Error */
.error-page { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; gap: 10px; color: #555; text-align: center; padding: 20px; }
...
.error-retry  { margin-top: 8px; background: #1a1a1a; border: 1px solid #333; color: #888; padding: 6px 16px; border-radius: 4px; cursor: pointer; font-size: 13px; }
...
/* Status */
.status-bar { display: flex; justify-content: space-between; align-items: center; padding: 2px 10px; background: #111; border-top: 1px solid #1a1a1a; font-size: 11px; color: #444; }
