...
function onNoMiresScroll(e) {
  ...
  if (nearBottom && !system.flags.readNoMires) {
    ...
    nextTick(() => {
      noMiresRef.value?.querySelectorAll('.line-final')
...
const noMiresFile = {
  name: 'no_mires.txt', icon: '📄', type: 'file',
  danger: true, isNoMires: true,
  flagOnOpen: 'readNoMires_opened', content: '',
};
...
const pathLabels = {
  ...
  privado: 'Privado', disco: 'Disco local (C:)', musica: 'Música', videos: 'Vídeos',
};
...
.no-mires-body {
  ...
  max-height: 420px; overflow-y: scroll; cursor: ns-resize;
}
