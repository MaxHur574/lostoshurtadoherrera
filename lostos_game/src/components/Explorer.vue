...
const filesystem = {
  root: [
    ...
    { name: 'Investigacion_Privada', icon: '🔒', type: 'inv-privada', path: 'investigacion', locked: true,
      lockHint: 'Pertenece a ev_local. La clave está en el correo.' },
  ],
  documentos: [
    { name: 'URGENTE_LEER.txt', icon: '📄', type: 'file', flagOnOpen: 'readUrgenteLeer',
      content: `Fecha: 14/05/2024 — 03:43\n\nNo sé cuánto tiempo me queda.\n\nLuna tiene hambre. No la he alimentado en todo el día.\nSiempre come a las 6. Ella nunca se olvida.\n\nSi alguien lee esto: ella sigue siendo la clave.\nBusca donde cuido a los que quiero.\n\n—E.V.\n\n[URGENTE — NO BORRAR]` },
    { name: 'Nota_al_margen.txt', icon: '📄', type: 'file', flagOnOpen: 'readNotaAlMargen',
      content: `Recordatorio — 07/03/2022\n\nSi olvido el acceso: revisar el correo primero.\nEl correo tiene todo lo que necesito para recordar.\n\n—E.V.` },
    { name: 'Entrevista_Pendiente.docx', icon: '📝', type: 'file', flagOnOpen: 'readEntrevistaPendiente',
      content: `BORRADOR — Entrevista pendiente\nPeriodista: E.V.\n\n—\n\nFuente anónima: "El programador desaparecido fue visto por última vez\ndurante el último trimestre de ese año."\n\n"Las fechas del expediente fueron alteradas antes de archivarlo.\nValeria insistía en que el patrón comenzó en octubre.\nLa línea exacta fue eliminada."\n\n[DOCUMENTO SIN TERMINAR]` },
    { name: 'Informe_caso_0047.docx', icon: '📝', type: 'file',
      content: `INFORME CLASIFICADO — CASO #0047\nFecha de apertura: 09/03/2022\nInvestigadora: E.V.\nEstado: COMPROMETIDO\n\n[Páginas 1–8 — CONTENIDO REDACTADO]\n\n——————\nNota al margen, página 9, escrita a mano:\n"Recordar: el 14 fue el día que empezó todo."\n——————\n\nNIVEL DE ACCESO REQUERIDO: ADMINISTRADOR` },
    { name: 'borrado_recuperado.txt', icon: '📄', type: 'file', danger: true,
  ...
  imagenes: [
    { name: 'sin_titulo_2.png', icon: '🖼️', type: 'file',
    ...
    { name: 'scan_doc_001.jpg', icon: '🖼️', type: 'file',
      content: `[Imagen — Sin previsualización]\n\nEscáner en blanco y negro.\nResolución: 300 DPI\nFecha: 11/03/2022` },
    { name: 'Captura_de_pantalla_082.png', icon: '🖼️', type: 'file', danger: true,
      content: `[Imagen — Contenido parcialmente legible]\n\nFecha: 02/03/2022 — 23:17\n\nTexto visible:\n"LOTE #0 — ESTADO: ACTIVO"\n"Conectado desde: [REDACTADO]"\n"Cámara: ONLINE"\n\n[Resto: ILEGIBLE]` },
  ],
  sistema: [
    { name: 'boot.log', icon: '📄', type: 'file', isBoot: true, mono: true,
  ...
  basura: [
    ...
    { name: 'cuenta_backup.tmp', icon: '📄', type: 'file', flagOnOpen: 'foundCuentaBackup',
  ...
  investigacion: [
    { name: 'Expediente_1410.pdf', icon: '📝', type: 'file',
    ...
    { name: 'Grabacion_04_Transcripcion.txt', icon: '📄', type: 'file', mono: true,
    ...
    { name: 'Contactos_Sospechosos.csv', icon: '📄', type: 'file', mono: true,
    ...
    { name: 'Chat_Interrumpido.log', icon: '📄', type: 'file', mono: true,
...
const fileMeta = {
  ...
  'cuenta_backup.tmp':           'Oculto entre archivos temporales — 14/05/2024',
};

const lockHints = {
  investigacion: 'Revisa el correo. La respuesta está ahí.',
};
...
// ── Breadcrumbs ───────────────────────────────────────────────────────────────
const pathLabels = {
  root: 'Mis archivos', documentos: 'Documentos', imagenes: 'Fotos',
  sistema: 'Sistema', basura: 'Basura', investigacion: 'Investigacion_Privada',
  disco: 'Disco local (C:)', musica: 'Música', videos: 'Vídeos',
};
...
function navigateTo(path) {
  ...
  if (path === 'root') {
    ...
    pathStack.value = pathStack.value.slice(0, pathStack.value.indexOf(path) + 1);
  } else {
...
function openItem(item) {
  ...
  if (item.type === 'file') {
    ...
    if (item.flagOnOpen && item.flagOnOpen in system.flags) system.setFlag(item.flagOnOpen);
...
function tryUnlock() {
  ...
  if (password.value === correct) {
    ...
    if (target === 'investigacion') {
      if (!system.flags.unlockedInvestigacion) system.setFlag('unlockedInvestigacion');
      showInvPrivada.value = true;
    } else {
      navigateTo(target);
    }
    ...
    if (unlockAttempts.value >= 5) {
      ...
      system.triggerEvent('notification', { message: 'Demasiados intentos fallidos. Actividad registrada.' });
    }
  }
}
</script>
...
.sidebar-item {
  ...
  padding: 5px 12px; color: #888; cursor: pointer;
  border-radius: 4px; margin: 1px 4px; font-size: 12px;
...
.breadcrumb {
  ...
  padding: 6px 12px; background: #151515;
  border-bottom: 1px solid #1e1e1e;
  font-size: 12px; color: #555; min-height: 30px;
}
...
.file-item {
  ...
  align-items: center; gap: 5px; padding: 8px 4px 6px;
...
.empty-folder { width: 100%; text-align: center; color: #333; font-size: 12px; padding: 30px 0; }

/* Statusbar */
.statusbar {
  padding: 4px 12px; background: #111; border-top: 1px solid #1e1e1e;
...
.dialog-title {
  background: #161616; padding: 9px 12px;
...
.dialog-error { margin: 0; color: #cc4444; font-size: 11px; display: flex; justify-content: space-between; }
.attempts { color: #664444; }
.dialog-btn {
  background: #1a3a5c; border: 1px solid #2a5a8c; color: #7ab8e0;
  padding: 7px 0; border-radius: 4px; cursor: pointer;
  font-size: 13px; font-family: 'Segoe UI', sans-serif; transition: background 0.15s;
}
...
.notepad-body {
  padding: 14px 16px; color: #aaa;
  font-family: 'Segoe UI', sans-serif; font-size: 13px;
  white-space: pre-wrap; min-height: 180px; max-height: 420px;
  overflow-y: auto; line-height: 1.75;
}
.body-mono      { font-family: 'Courier New', monospace; font-size: 12px; }
.body-corrupted { color: #886655; font-family: 'Courier New', monospace; font-size: 12px; }
.notepad-statusbar {
