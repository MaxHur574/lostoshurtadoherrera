<template>
  <Teleport to="body">
    <div class="endgame-root" ref="rootRef">
        ...
        <div v-if="showAdminChat" class="admin-chat-window" ref="chatWindowRef">
          ...
          <div class="ac-body">
            <div class="ac-messages">
              <div v-for="(msg, i) in visibleChatLines" :key="i" class="ac-msg">
      ...
      <!-- ══ EPÍLOGO ══ -->
      <Transition name="finale-fade">
        <div v-if="showEpilogue" class="epilogue-screen">
          <div class="finale-scanlines" />
          <div class="epilogue-content">
            <div
              v-for="(line, i) in epilogueLines"
              :key="i"
              class="epilogue-line"
              :class="{ visible: visibleEpilogueLines > i }"
            >
              {{ line }}
            </div>
          </div>
      ...
      <!-- ══ PERIÓDICO ══ -->
      <Transition name="finale-fade">
        <div v-if="showPeriodico" class="periodico-screen">
          <div class="finale-scanlines" />
          <div class="periodico-wrap">
            <img
              :src="periodicoImg"
              class="periodico-img"
              alt="El Caso — Diario Policial"
            />
            <div class="periodico-caption">CASO 0048 — EN CURSO</div>
          </div>
        </div>
      </Transition>
    </div>
...
import periodicoImg from "../assets/periodico.jpg";

const showPeriodico = ref(false);
...
// ── Epílogo ───────────────────────────────────────────────────────────────────
const visibleEpilogueLines = ref(0);
const epilogueLines = [
  "Tu escritorio ya fue copiado.",
  "Tus archivos ya fueron indexados.",
  "Tu cámara ya está activa.",
  "",
  "En unos minutos",
  "otro usuario recibirá un correo sin asunto.",
  "Un enlace.",
  "Un archivo.",
  "Un pequeño error de envío.",
  "",
  "Y hará exactamente lo mismo que tú.",
];
...
function corruptChat() {
  ...
  textEls.forEach((el) => {
    ...
    el.innerHTML = chars
      .map(
        (c) => `<span class="corrupt-char">${c === " " ? "&nbsp;" : c}</span>`,
      )
      .join("");
...
async function onTransmitClick() {
  ...
  // pausa después de CASO 0047 — ARCHIVADO
  await sleep(3500);

  // epílogo
  showEpilogue.value = true;
  for (let i = 0; i < epilogueLines.length; i++) {
    await sleep(epilogueLines[i] === "" ? 600 : 1100);
    visibleEpilogueLines.value = i + 1;
  }

  await sleep(2000);
  console.log("🔴 showPeriodico activado", periodicoImg);
  showPeriodico.value = true;
}
...
.periodico-img {
  ...
  animation: periodico-appear 1.5s ease-out;
}
@keyframes periodico-appear {
  from {
    opacity: 0;
...
.periodico-caption {
  ...
  color: #441111;
  letter-spacing: 3px;
  animation: periodico-appear 2s ease-out;
}
</style>
