    ...
    <Transition name="start-menu">
      <div v-if="showStart" class="start-menu" @click.stop>
        <div class="start-header">
          <div class="start-user">👤 usuario@lost.sys</div>
        </div>
...
const emit = defineEmits(["open-explorer", "open-mail", "open-browser", "restore-app"]);
...
function updateClock() {
  const now = new Date();
  time.value = now.toLocaleTimeString("es-CL", { hour: "2-digit", minute: "2-digit" });
  date.value = now.toLocaleDateString("es-CL", { day: "2-digit", month: "2-digit", year: "numeric" });
}
